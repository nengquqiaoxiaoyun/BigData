...
1. 在**NameNode**节点`$HADOOP_HOME/etc/hadoop`目录下创建`whitelist`和`blacklist`文件，将受信任的节点写入`whitelist`
...
4. 配置NameNode节点（hadoop102）和ResouceManager(hadoop103)节点到新节点的ssh无秘钥登录

```shell
cd ~
cd .ssh
ssh-copy-id hadoop 105
```

5. 将新节点添加至白名单（所有节点）并刷新（非第一次添加的情况下）
